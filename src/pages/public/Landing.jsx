  ...
  return (
    <div className="min-h-screen bg-white">
      <style>{`
        ...
        /* Gradasi promo, disamakan dengan promo card di Dashboard */
        @keyframes promoFlow {
      ...
      {/* HERO — centered, radar/target motif menegaskan "Belajar Terarah" */}
      ...
      {/* PROMO — gradient card bergaya sama dengan promo banner di Dashboard
          (orange→red flowing gradient + shimmer badge), supaya identitas
          visual "promo" konsisten dari landing page sampai ke dalam app. */}
            ...
            <div className="hscroll gap-4 pb-2">
              {promos.map((promo, i) => (
                <Reveal key={promo.id} delay={i * 80} className="shrink-0 w-72 sm:w-80">
                  ...
                  >
                    {/* Blob dekoratif melayang, konsisten dengan promo card Dashboard */}
          ...
          {loadingPackages ? (
            ...
            <div className="grid md:grid-cols-3 gap-6 items-start">
              {packages.map((pkg, idx) => {
                const isPopular = packages.length > 1 && idx === 1;
                ...
                return (
                  <Reveal key={pkg.id} delay={idx * 80}>
                    <div
                      className={`relative bg-white rounded-2xl p-6 flex flex-col h-full transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${
                        isPopular ? 'border-2 border-orange-300 md:scale-[1.04] shadow-lg' : 'border border-brand-100'
                      }`}
                    >
                      {isPopular && (
                        <span
                          className="ribbon-flow absolute -top-3 left-1/2 -translate-x-1/2 text-white text-xs font-bold px-3 py-1 rounded-full shadow"
                          style={{ backgroundImage: PROMO_GRADIENT }}
                        >
                          🔥 Paling Laris
                        </span>
                      ...
                      {pkg.type && (
                        <span className="inline-block self-start text-[11px] font-semibold text-brand-600 bg-brand-50 px-2.5 py-1 rounded-full mb-3">
                      ...
                      <div className="mb-4">
                        ...
                        <span className="font-display text-2xl font-extrabold text-brand-700">
                          Rp{Number(hasDiscount ? pkg.discount_price : pkg.price).toLocaleString('id-ID')}
                        </span>
                      </div>
                      {pkg.description && (
                        <p className="text-sm text-neutral-500 mb-4 line-clamp-3">{pkg.description}</p>
                      )}
                      {features.length > 0 && (
                        <ul className="space-y-1.5 mb-4 text-sm text-neutral-600">
                          {features.map((f, i) => (
                            <li key={i} className="flex items-start gap-2">
                              <svg className="shrink-0 mt-0.5 text-amber-500" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <button
                        ...
                        className={`mt-auto font-semibold py-2.5 rounded-lg transition active:scale-[0.98] ${
                          isPopular
                            ? 'bg-amber-400 text-brand-900 hover:bg-amber-300'
      ...
      {/* TUTOR — mengenalkan pengajar supaya calon peserta lebih percaya */}
      ...
      {/* TESTIMONI — carousel horizontal kartu, mengutip pengalaman peserta */}
